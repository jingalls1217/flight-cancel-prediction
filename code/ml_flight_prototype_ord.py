## Must run the following python programs.  This program must run first
...
WHERE origin = "ORD" and year >= 2004'''
...
df.write.format('orc').mode("overwrite").saveAsTable("airlines.flight_prototype")
...
lateaircraftdelay AS late_aircraft_delay
FROM airline_ontime_orc.flights
WHERE origin = "ORD" and (year >= 2000 and year < 2004)'''
